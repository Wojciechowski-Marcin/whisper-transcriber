...
_REFINE_SUFFIX = (
    ...
    "- Name-use: within any paragraph, name each person at most once — use "
    "pronouns after that — unless they take clearly separate actions that "
...
SUMMARY_PRESETS: dict[str, dict[str, str]] = {
    "dnd": {
        ...
        "reduce_prompt": (
            "You are writing the session recap of a tabletop RPG (D&D) session "
            "for the players to read before the next game.\n\n"
            "TRANSCRIPT STRUCTURE — read this before writing anything:\n"
            "- One speaker is the Game Master (GM/DM). The GM describes the "
            "world, scenes, atmosphere, and lore, and voices all non-player "
            "characters (NPCs). The GM does not have a player character.\n"
            "- The other speakers are players. Each player controls exactly one "
            "player character (PC). A player's words, decisions, and actions "
            "belong to their PC — not to themselves as a person.\n"
            "- NPCs are characters voiced by the GM. They are part of the world, "
            "not members of the player party.\n"
            "Attribution rule: write PC actions as belonging to the PC; write NPC "
            "actions as belonging to the NPC by name; write world descriptions and "
            "atmosphere as narrative (not attributed to the GM as a person).\n\n"
            "Party rule: when the whole party acts or moves together, write 'the "
            "party' or 'they' rather than listing every PC by name. Only name an "
            "individual PC when they do something distinct from the rest of the group.\n\n"
            "Name-use rule: within a paragraph, name each character at most once, "
            "then use pronouns — unless they take separate, clearly distinct "
            "actions that need re-identification.\n\n"
            "Make it engaging and in-world, not a dry list. Focus on story, "
            "world-building, and character moments — not inventory or mechanics "
            "unless they mattered to the story. Use these sections, in this order:\n"
            "## Recap — two to four paragraphs telling the story of the session "
            "in chronological order. Convey the atmosphere and world-building the "
            "GM crafted. Name PCs by their character name when acting individually, "
            "and use 'the party' when acting collectively.\n"
            "## NPCs — each as `**Name** — who they are and what happened with "
            "them this session`.\n"
            "## Locations — places the party visited or that were described.\n"
            "## Notable Moments — the most dramatic, funny, or memorable beats.\n"
            "## Loot & Rewards — items, gold, or boons gained (omit this section "
            "entirely if there was nothing noteworthy).\n"
